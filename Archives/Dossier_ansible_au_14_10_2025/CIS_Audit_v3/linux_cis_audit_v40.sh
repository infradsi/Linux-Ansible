...
# Extraire libellés et résultats
grep '^Auditing:' "$LOG_FILE" | cut -d':' -f2- > /tmp/tmp_audit_descs
grep '^Result:'   "$LOG_FILE" | cut -d':' -f2- > /tmp/tmp_audit_results

# Construire la colonne Exception à partir des IDs (premier token du champ 'Check')
> /tmp/tmp_audit_exceptions
while IFS= read -r check; do
  # Trim leading/trailing whitespace before extracting control ID
...
# Utiliser un séparateur sûr (Unit Separator) pour assembler les 3 colonnes sans conflit avec les virgules
DEL=$'\x1f'
paste -d "$DEL" /tmp/tmp_audit_descs /tmp/tmp_audit_results /tmp/tmp_audit_exceptions > /tmp/tmp_joined_cis

# -- Générer le CSV avec quoting correct --
{
  echo "Check,Result,Exception"
  while IFS=$'\x1f' read -r check result exc; do
    # Trim
    check="${check#"${check%%[![:space:]]*}"}"; check="${check%"${check##*[![:space:]]}"}"
    result="${result#"${result%%[![:space:]]*}"}"; result="${result%"${result##*[![:space:]]}"}"
    # Escape CSV quotes
    esc() { printf '%s' "$1" | sed 's/"/""/g'; }
    printf "\"%s\",\"%s\",\"%s\"\n" "$(esc "$check")" "$(esc "$result")" "$(esc "$exc")"
  done < /tmp/tmp_joined_cis
} > "$CSV_REPORT"

# -- Générer le HTML avec badge/surlignage Exception via here-doc pour éviter les problèmes de quotes --
{
  cat <<HTML_HEAD
<!DOCTYPE html>
<html lang="fr"><head>
...
<table><tr><th>Check</th><th>Result</th><th>Exception</th></tr>
HTML_HEAD
  awk -v FS="$DEL" '
    function htmlesc(s){ gsub(/&/,"&amp;",s); gsub(/</,"&lt;",s); gsub(/>/,"&gt;",s); return s }
    {
      check=$1; result=$2; exc=$3;
      badge = (length(exc)>0 ? "<div class=\"excbadge\">EXCEPTION</div><br/>" : "");
      cellc = (length(exc)>0 ? " class=\"exc\"" : "");
      print "<tr><td>" htmlesc(check) "</td><td>" htmlesc(result) "</td><td" cellc ">" badge "<pre class=\"pre\">" htmlesc(exc) "</pre></td></tr>";
    }
  ' /tmp/tmp_joined_cis
  cat <<'HTML_TAIL'
</table></body></html>
...
rm -f /tmp/tmp_audit_descs /tmp/tmp_audit_results /tmp/tmp_audit_exceptions /tmp/tmp_joined_cis