...
is_interactive_shell() {
  local shell="$1"
  [[ "${INCLUDE_NONINTERACTIVE:-0}" -eq 1 ]] && return 0
  if [[ -r /etc/shells ]]; then
    grep -qxF "$shell" /etc/shells 2>/dev/null
  else
    case "$shell" in */bash|*/zsh|*/sh|*/ksh) return 0 ;; *) return 1 ;; esac
  fi
}
...
ARGS=(); while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -s) SKIP_USERS="${2:-}"; shift 2;;
    -I) INCLUDE_SYSTEM=1; shift;;
    -M) MAXUID="${2:?}"; shift 2;;
    -i) INCLUDE_NONINTERACTIVE=1; shift;;
    -v) VERBOSE=1; shift;;
    -h|--help) echo "Usage: $0 [-d DAYS] [-o CSV] [--json FILE] [-s list] [-I] [-M MAXUID] [-i] [-v]"; exit 0;;
    --) shift; break;;
...
NOW=$(date +%s)
UID_MIN=1000; [[ -r /etc/login.defs ]] && { v=$(awk '/^\s*UID_MIN/{print $2}' /etc/login.defs | tail -n1); [[ -n "${v:-}" ]] && UID_MIN="$v"; }
...
while IFS=: read -r user pw uid gid gecos home shell; do
  ...
  if [[ -n "$JSON_OUT" ]] && [[ -w "$JSON_OUT" ]]; then
    printf '  {"user":"%s","uid":%s,"gid":%s,"threshold_days":%s,"days_since_last_activity":%s,' "$(json_escape "$user")" "$uid" "$gid" "$DAYS" "$DIFF_DAYS"
    printf '"last_login":"%s","last_password_change":"%s","activity_source":"%s",' "$(json_escape "$LLOGIN_H")" "$(json_escape "$LPASS_H")" "$(json_escape "$SRC")"
    printf '"status":"%s","account_state":"%s","shell":"%s","home":"%s"},\n' "$(json_escape "$STATUS")" "$(json_escape "$ACC_STATE")" "$(json_escape "$shell")" "$(json_escape "$home")"
  fi
done < /etc/passwd
