...
LOG_FILE="/tmp/cis_audit_report_$(hostname -f)_$(date +%F).log"
...
if grep -q 'arch.*b32' /sys/kernel/tracing/events/syscalls/; then
    ARCHS+=("b32")
fi
...
if rpm -q aide >/dev/null 2>&1 || dpkg -l | grep -q "^ii  aide"; then
...
DUPES=$(cut -d: -f0 /etc/passwd | sort | uniq -d)
if [ -z "$DUPES" ]; then
...
DUPES=$(cut -d: -f0 /etc/group | sort | uniq -d)
if [ -z "$DUPES" ]; then
...
CSV_REPORT="/tmp/cis_audit_report_$(hostname -f)_$(date +%F).csv"

HTML_REPORT="/tmp/cis_audit_report_$(hostname -f)_$(date +%F).html"
...
echo "<html><body><h2>CIS Audit Report for host $(hostname -f)</h2><table border=1><tr><th>Check</th><th>Result</th></tr>" > "$HTML_REPORT"
