...
is_interactive_shell() {
  ...
  if [[ -r /etc/shells ]]; then grep -qxF "$shell" /etc/shells 2>/dev/null; else
...
if [[ -n "$JSON_OUT" ]]; then tmp="$(mktemp)"; if [[ -s "$JSON_OUT" ]]; then sed '$ s/},/}/' "$JSON_OUT" > "$tmp" || true; fi; echo "]" >> "$tmp"; mv "$tmp" "$JSON_OUT"; fi
