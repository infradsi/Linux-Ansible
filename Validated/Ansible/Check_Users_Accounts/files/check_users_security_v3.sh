...
PAM_PWQUALITY=$(grep -E "pam_pwquality|pam_cracklib" /etc/pam.d/system-auth /etc/pam.d/password-auth /etc/pam.d/common-password 2>/dev/null)
...
while IFS=: read -r user _ uid gid _ home shell; do
    if [ "$uid" -ge 1000 ] || { [ "$uid" -ge 100 ] && [ "$uid" -lt 1000 ]; }; then
        aging_info=$(LC_ALL=C chage -l "$user")
