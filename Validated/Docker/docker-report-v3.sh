# Script for documenting Docker containers on the current host
# MGH / Version 3.0
#
...
# Fonction pour formater la date ISO en format humain
format_date() {
    raw_date="$1"
    if [[ "$raw_date" == "0001-01-01T00:00:00Z" || -z "$raw_date" ]]; then
        echo "N/A"
    else
        date -d "$raw_date" "+%d/%m/%Y %H:%M:%S"
    fi
}
...
<head>
    <meta charset="UTF-8">
    <title>Rapport Docker – $SERVER_NAME – $DATE</title>
    <style>
        body {
...
<body>
<h1>📦 Rapport Docker – Serveur : $SERVER_NAME</h1>
...
for container in $containers; do
    ...
    started_raw=$(docker inspect --format='{{.State.StartedAt}}' "$container")
    finished_raw=$(docker inspect --format='{{.State.FinishedAt}}' "$container")

    started_at=$(format_date "$started_raw")
    finished_at=$(format_date "$finished_raw")
